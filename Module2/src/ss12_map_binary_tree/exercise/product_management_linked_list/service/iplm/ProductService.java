package ss12_map_binary_tree.exercise.product_management_linked_list.service.iplm;

import ss12_map_binary_tree.exercise.product_management_linked_list.model.Product;
...
public class ProductService implements IProductService {
    ...
    public void edit() {
        ...
        findProductIndex = productList.indexOf(findProductByID());
        if(findProductIndex > -1) {
            Product productUpdate = createProduct("Nhập các thông tin cần chỉnh sửa: ");
            productList.get(findProductIndex).setId(productUpdate.getId());
            productList.get(findProductIndex).setName(productUpdate.getName());
            productList.get(findProductIndex).setName(productUpdate.getName());
    ...
    public void remove() {
        if(productList.size() > 0) {
            ...
            findProductIndex = productList.indexOf(findProductByID());
            if(findProductIndex > -1) {
                productList.remove(findProductIndex);
